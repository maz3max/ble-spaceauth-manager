...
def confirm_authentication():
    ...
    mqtt_pub(topic=args.topic, payload=args.msg,
             qos=args.qos, hostname=args.host)
...
Coin = namedtuple("Coin", "address battery_level", defaults=["00:00:00:00:00:00", 0])
        ...
        if self.config_mode:
            ...
            for bond, skey in zip(self.bonds, self.spacekeys):
                ...
                if bond[0] not in self.db.coins \
                    ...
                    logging.warn(f"deleting {bond[0]}")
                    await self._wait_until_done()
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--topic', default='Things/Door',
        help='MQTT Topic to publish to')
    parser.add_argument('--host', default='localhost',
                        help='MQTT Host to publish to')
    parser.add_argument('--port', default=1883, type=int,
                        help='MQTT Host Port to publish to')
    parser.add_argument('--qos', default=2, type=int,
                        help='QOS of MQTT message')
    parser.add_argument('--msg', default=b"open",
                        type=bytes, help='MQTT Message payload')
    parser.add_argument('--test', action='store_true', default=False,
                        help='Do not send MQTT messages on successfull auth.')
